...
const ROLES: Record<RoleId, { name: string; english: string; action: string; alignment: "crew" | "spy" }> = {
  ...
  security: { name: "보안 책임자", english: "SECURITY DIRECTOR", action: "기본 조사 또는 보안 비밀 조회", alignment: "crew" },
...
export default function MultiplayerApp({ onExit }: { onExit: () => void }) {
  ...
  const dossier = <aside className={`mp-dossier ${role.alignment}`}>
    ...
    <SymbolStrip symbols={view.secret.totals}/>
  ...
  if (view.room.status === "action") {
    ...
    return frame(<div className="mp-console"><p className="mp-kicker">내 차례 · 남은 시간 {clockText}</p><h2>이번 차례에 무엇을 할까요?</h2><p className="mp-help">아래 행동 중 하나만 고릅니다. 기본 조사는 질문까지 정한 뒤 바로 결과가 공개됩니다. 체포는 자신 있을 때 언제든 고를 수 있지만, 하나라도 틀리면 스파이가 즉시 이깁니다.</p><div className="mp-turn-choices"><button type="button" className={crewChoice === "basic" ? "selected" : ""} onClick={() => setCrewChoice("basic")}>기본 조사</button>{specialAvailable ? <button type="button" className={crewChoice === "special" ? "selected" : ""} onClick={() => setCrewChoice("special")}>{specialLabel}</button> : null}{view.secret.roleId === "security" ? <button type="button" className={crewChoice === "query" ? "selected" : ""} onClick={() => setCrewChoice("query")}>보안 비밀 조회</button> : null}<button type="button" className={crewChoice === "arrest" ? "selected danger" : "danger"} onClick={() => setCrewChoice("arrest")}>스파이 체포</button></div>
      ...
      {crewChoice === "query" && view.secret.roleId === "security" ? <div className="mp-confidential"><h3>보안 비밀 조회</h3><p>광물 종류와 기준 개수를 정하면 스파이가 실제로 그만큼 가지고 있는지 보안 책임자인 나에게만 O/X가 보입니다. 확정한 질문은 바꿀 수 없고, 다른 플레이어에게 결과가 자동 공개되지 않습니다.</p><SymbolPicker value={querySymbol} onChange={setQuerySymbol}/><label className="mp-range"><span>기준 개수</span><input type="number" min="1" max="20" value={queryThreshold} onChange={(event) => setQueryThreshold(Number(event.target.value))}/><b>{queryThreshold}개 이상</b></label><button className="mp-primary" type="button" disabled={busy} onClick={() => void act("action", { action: { type: "query", symbol: querySymbol, threshold: queryThreshold } })}>질문 잠금 · 나만 결과 보기</button></div> : null}
