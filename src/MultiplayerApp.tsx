...
function LocationPicker({ value, onChange, disabled }: { value: number; onChange: (value: number) => void; disabled?: number | null }) {
  return <div className="mp-location-grid">{LOCATIONS.map(([id, name, english]) => <button type="button" disabled={id === disabled} className={value === id ? "selected" : ""} onClick={() => onChange(id)} key={id}><img src={locationImage(id)} alt="" loading="lazy" /><b>{String(id).padStart(2, "0")}</b><span>{name}<small>{id === disabled ? "LOCKDOWN COOLDOWN" : english}</small></span></button>)}</div>;
}
...
export default function MultiplayerApp({ onExit }: { onExit: () => void }) {
  ...
  const dossier = <aside className={`mp-dossier ${role.alignment}`}><small>{role.english}</small><img className="mp-role-art" src={roleImage(view.secret.roleId)} alt={`${role.name} 인물 일러스트`} /><h2>{role.name}</h2><p>{role.action}</p><div className="mp-role-card"><small>역할 카드 아이템</small><SymbolStrip symbols={roleSymbols}/></div><b>내 카드 4장 총 심볼 {total(view.secret.totals)}개</b>{view.me.eliminated ? <div className="mp-out">OUT · 관전 모드</div> : null}{view.secret.roleId === "spy" ? <div className="mp-spy-log"><small>PRIVATE OMEGA LOG</small><p>{view.secret.privateLog}</p><strong>{state!.destroyed}/5</strong></div> : null}<div className="mp-hand"><small>내 안전 구역 카드</small>{view.secret.hand.map((card) => <article key={card.id}><img src={locationImage(card.id)} alt="" loading="lazy" /><span>{String(card.id).padStart(2,"0")} · {locationName(card.id)}</span><SymbolStrip symbols={card.symbols}/></article>)}</div></aside>;
