...
function SymbolStrip({ symbols, showPool = false }: { symbols: Partial<Symbols>; showPool?: boolean }) {
  return <div className="mp-symbols">{symbolKeys.map((key) => <span className={`mineral-${key}`} title={SYMBOLS[key].name} key={key}>{SYMBOLS[key].icon} {Number(symbols[key] ?? 0)}{showPool ? `/${MINERAL_POOL_SIZE}` : ""}</span>)}</div>;
}
