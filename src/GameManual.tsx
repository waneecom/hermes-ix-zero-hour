...
const institutions = [
  [1, "제1 메인 리액터실", "ϟ ϟ ϟ"], [2, "양자 연산 코어실", "◉ ⬡ ⬡"], [3, "중력 제어 장치실", "ϟ ⬡"],
  [4, "생명유지 산소실", "✦ ✦"], [5, "서브 통신 중계탑", "◉ ⬡"], [6, "함교 항법 콘솔실", "◆ ◆ ⬡"],
  [7, "바이오 큐브 연구실", "◉ ✦ ✦"], [8, "비상 동력 배전반", "ϟ ✦"], [9, "격벽 보안 통제실", "◆ ◆"],
  [10, "센서 레이더 돔", "◉ ◉"], [11, "보조 플라즈마 추진실", "◆ ϟ"], [12, "선외 탈출 포드실", "◆ ✦"],
  [13, "암흑물질 차폐고", "◉ ⬡"],
] as const;
...
export default function GameManual({ onClose }: { onClose: () => void }) {
  ...
  const pages: Array<{ short: string; title: string; subtitle: string; body: ReactNode }> = [
    {
      ...
      body: <>
        ...
        <div className="rule-two-columns">
          ...
          <RuleBlock title="스파이 1명의 목표" tone="secret"><p>자기 화면에만 보이는 목표 구역을 공격하세요. 막히지 않은 파괴 공격을 <b>7번</b> 성공시키거나, 승무원이 잘못된 체포를 선언하면 즉시 승리합니다.</p></RuleBlock>
        </div>
        ...
        <p className="manual-tip"><b>한 줄 요약:</b> 승무원은 공개 조사로 두 개의 정답을 찾고, 스파이는 들키기 전에 비밀 공격 7번을 성공시키는 게임입니다.</p>
      </>,
    },
    {
      short: "준비", title: "카드·광물·게임 준비", subtitle: "웹사이트가 섞기와 배분을 자동으로 처리합니다.",
      body: <>
        <RuleBlock title="카드는 정확히 17장입니다"><p><b>역할 카드 4장</b>은 수석 조종사, 수석 과학자, 보안 책임자, 기계 관리사(스파이)입니다. 역할 카드는 능력만 정하고 광물 질문에는 넣지 않습니다. <b>장소 카드 13장</b> 중 1장은 비밀 파괴 목표가 되고, 나머지 12장은 네 명에게 3장씩 나뉩니다.</p></RuleBlock>
        <RuleBlock title="장소 카드에 광물 30개가 있습니다"><p>다섯 종류가 각각 정확히 6개씩 장소 카드 13장에 놓입니다. 역할 카드에는 조사용 광물이 없습니다. 설명과 실제 게임은 아래의 같은 기호를 사용합니다.</p><div className="manual-minerals">{minerals.map(([icon, name, text]) => <span key={name}><b>{icon}</b><strong>{name} · 전체 6개</strong><small>{text}</small></span>)}</div><p className="rule-note">내 정보의 “◉ 3/6”은 전체 장소 카드에 있는 센서 결정 6개 중, 광물 질문에서 내 것으로 계산되는 카드에 3개가 있다는 뜻입니다. 네 사람의 질문 수량을 모두 합치면 광물마다 정확히 6/6이 됩니다.</p></RuleBlock>
        ...
        <RuleBlock title="질문에서 누구 카드로 계산하나요?" tone="warning"><p><b>승무원:</b> 자신이 받은 안전 장소 카드 3장만 계산합니다.</p><p><b>스파이:</b> 안전 장소 카드 3장과 비밀 파괴 목표 카드 1장을 합쳐 네 장으로 계산합니다. 파괴할 때는 목표 카드지만, 모든 광물 질문과 보안 비밀 조회에서는 스파이 자신의 카드입니다.</p></RuleBlock>
        <RuleBlock title="바뀌는 것과 바뀌지 않는 것"><p><b>매 게임 무작위:</b> 역할을 받는 사람, 비밀 파괴 목표 1장, 각자 받는 안전 장소 카드 3장, 카드가 화면에 나오는 순서.</p><p><b>항상 고정:</b> 장소별 광물과 전체 광물 30개.</p></RuleBlock>
    ...
    },
    {
      ...
      body: <>
        ...
        <RuleBlock title="화면 왼쪽의 내 정보"><p>내 차례와 대기 화면에서 역할, 안전 장소 카드 3장, 질문에 적용되는 광물 수를 다시 볼 수 있습니다. 승무원은 안전 장소 3장만, 스파이는 안전 장소 3장과 비밀 목표 1장을 합산해 표시합니다. 스파이는 파괴 성공 횟수도 같은 비밀 영역에서 확인합니다.</p></RuleBlock>
      </>,
    },
    {
      ...
      body: <>
        ...
        <div className="rule-two-columns">
          <RuleBlock title="한 명 조사"><p>자신을 제외한 살아 있는 한 명을 고릅니다. 승무원이면 안전 장소 3장, 스파이면 안전 장소 3장과 비밀 목표 1장에 있는 선택 광물을 서버가 합산해 O/X 하나를 즉시 공개합니다.</p></RuleBlock>
          <RuleBlock title="모두 조사"><p>살아 있는 모든 사람을 한꺼번에 확인합니다. 질문한 사람 자신과 스파이도 포함하며, 사람별 O/X가 즉시 공개됩니다.</p></RuleBlock>
        </div>
        <RuleBlock title="거짓말은 없습니다" tone="warning"><p>기본 조사에는 말로 대답하는 단계가 없습니다. 서버가 실제 카드 수를 계산하므로 승무원도 스파이도 답을 바꿀 수 없습니다. 예전 규칙의 “스파이는 전체 질문에서 거짓말 가능”은 사용하지 않습니다.</p><p className="rule-note"><b>대화 규칙:</b> 공개된 O/X, 공개 카드, 광물 숫자를 인용할 때는 누구도 거짓말할 수 없습니다. 비밀 역할이나 비공개 카드 질문에는 대답을 거부할 수 있고, “나는 저 사람이 의심된다” 같은 의견과 추측은 자유입니다.</p></RuleBlock>
        ...
        <RuleBlock title="3라운드 긴급 신뢰 스캔" tone="secret"><p>3라운드 판정이 끝나면 서버가 네 사람의 <b>안전 장소 카드 중 한 장씩</b>을 무작위로 골라 모두에게 공개합니다. 스파이에게서도 비밀 목표가 아닌 안전 장소 한 장만 공개됩니다.</p><ul><li>서버가 공개한 확정 정보라 누구도 거짓말하거나 바꿀 수 없습니다.</li><li>승무원은 목표 후보와 광물 분포를 더 쉽게 좁힐 수 있습니다.</li><li>스파이도 공개된 카드로 역추적 후보를 좁힐 수 있어 완전히 일방적인 도움은 아닙니다.</li><li>한 게임에 한 번만 실행되고 공개 기록은 게임이 끝날 때까지 남습니다.</li></ul></RuleBlock>
        <RuleBlock title="좋은 질문 예시"><ul><li>처음에는 모두 조사로 2개 이상을 물어 전체 분포를 넓게 확인합니다.</li><li>다음에는 의심되는 사람에게 같은 광물을 3개 이상, 4개 이상으로 좁혀 실제 수의 범위를 찾습니다.</li><li>네 사람의 광물 수를 합치면 종류마다 6개이므로, 다른 세 명의 수를 알아내면 마지막 사람의 수를 계산할 수 있습니다.</li><li>스파이 수량에는 비밀 목표 카드도 들어간다는 점을 이용해 목표 후보를 좁힙니다.</li></ul></RuleBlock>
      </>,
    },
    {
      ...
      body: <>
        ...
        <RuleBlock title="수석 과학자 — 현장 확인"><p>짝수 라운드에 구역 하나를 고릅니다. 스파이가 그 라운드에 파괴 공격을 했고, 고른 곳이 진짜 목표라면 <b>O</b>입니다. 그 외에는 <b>X</b>입니다.</p><ul><li>O는 고른 구역이 진짜 목표이며 이번 라운드에 공격받았다는 강력한 단서입니다.</li><li>O가 나오면 스파이의 안전 장소 3장과 비밀 목표 1장에 든 <b>광물 전체 개수</b>도 공개됩니다.</li><li>목표 구역이 맞아도 스파이가 조용히 있었다면 X입니다.</li><li>조종사가 공격을 막았더라도 스파이가 목표를 공격했다면 현장 확인은 O가 됩니다.</li></ul></RuleBlock>
        <RuleBlock title="보안 책임자 — 보안 비밀 조회" tone="secret"><p>보안 책임자는 매 차례 <b>기본 조사, 보안 비밀 조회, 체포</b> 중 하나를 선택합니다. 조회에서는 광물 한 종류와 “몇 개 이상”을 정합니다. 서버가 스파이의 안전 장소 3장과 비밀 목표 1장을 확인해 O/X를 <b>보안 책임자 화면에만</b> 즉시 보여 줍니다.</p><ul><li>질문을 확정한 뒤에는 광물과 기준 개수를 바꿀 수 없습니다.</li><li>다른 플레이어에게 결과가 자동으로 공개되지 않습니다. 말로 나눌지는 보안 책임자가 판단합니다.</li><li>이 행동을 쓴 차례에는 기본 조사를 함께 할 수 없습니다.</li><li>화면의 비밀 결과 영역에서 마지막 조회 결과를 다시 확인할 수 있습니다.</li></ul></RuleBlock>
      </>,
    },
    {
      short: "스파이", title: "스파이의 목표와 세 행동", subtitle: "목표 구역과 파괴 횟수는 오직 스파이 화면에만 보입니다.",
      body: <>
        <RuleBlock title="비밀 정보" tone="secret"><p>기계 관리사는 사실 처음부터 승무원으로 잠복한 휴머노이드 스파이입니다. 게임이 시작되면 왼쪽 비밀 카드에 <b>파괴 목표 구역</b>과 <b>성공한 파괴 횟수 / 7</b>이 표시됩니다. 이 두 정보는 공개 방 데이터에서도 제외되어 승무원은 볼 수 없습니다.</p></RuleBlock>
        <RuleBlock title="1. 파괴 공격"><p>비밀 목표를 공격합니다. 같은 라운드 조종사의 잠금이 목표에 걸려 있으면 실패하고, 아니면 성공 횟수가 1 늘어납니다. 누적 7번 성공하면 즉시 스파이가 승리합니다. 막힌 공격과 조용히 있기는 7회에 포함되지 않습니다. 성공 횟수는 게임이 끝나도 스파이 화면에만 숫자로 표시됩니다.</p></RuleBlock>
    ...
    },
    {
      ...
      body: <>
        ...
        <RuleBlock title="모든 승리 조건 정리"><ul><li><b>승무원 승리:</b> 정확한 스파이와 목표 구역을 함께 맞혀 체포.</li><li><b>스파이 승리:</b> 막히지 않은 파괴 공격 7번 성공.</li><li><b>스파이 승리:</b> 승무원이 잘못된 체포 선언.</li><li><b>스파이 승리:</b> 역추적으로 살아 있는 승무원을 모두 탈락시킴.</li></ul></RuleBlock>
    ...
    },
    {
      ...
      body: <>
        <RuleBlock title="시작 전 9가지"><ol><li>정확히 4명이 각자 다른 기기로 같은 방에 들어왔나요?</li><li>공개 장소 도감 13장과 고정 광물을 확인했나요?</li><li>다른 사람에게 역할과 자기 안전 장소 3장을 보여 주지 않았나요?</li><li>장소 카드에는 광물 5종이 각각 6개, 총 30개라는 것을 알았나요?</li><li>스파이의 질문 수량에는 비밀 파괴 목표 카드도 포함된다는 것을 알았나요?</li><li>자기 차례는 2분이며 행동 하나만 고른다는 것을 알았나요?</li><li>기본 조사 결과는 즉시 공개되고 누구도 거짓말할 수 없다는 것을 알았나요?</li><li>목표와 파괴 횟수는 스파이에게만 보인다는 것을 알았나요?</li><li>체포는 사람과 구역을 둘 다 맞혀야 한다는 것을 알았나요?</li></ol></RuleBlock>
        ...
        <RuleBlock title="왜 7회와 신뢰 스캔을 함께 쓰나요?"><p>승무원은 3라운드 신뢰 스캔으로 거짓말 없는 카드 단서를 네 장 얻지만, 체포 답 두 개 중 하나만 틀려도 즉시 패배합니다. 스파이는 7회의 성공 공격이 필요하고, 동시에 공개된 카드로 상대 기관 카드 3장을 추리해 역추적할 수 있습니다. 조종사는 짝수 라운드에만 방어하므로 홀수 라운드에는 공격 기회가 생깁니다.</p><p className="rule-note"><b>권장 점검:</b> 이 규칙으로 10경기를 진행해 승무원 승률이 65%를 넘으면 신뢰 스캔을 4라운드로 늦추고, 35%보다 낮으면 첫 잘못된 체포는 선언자만 탈락하도록 완화하는 방식이 좋습니다.</p></RuleBlock>
        <RuleBlock title="이전 규칙 방이라는 안내가 나와요"><p>규칙 4.5 이전에 만든 방입니다. 장소 광물과 스파이 목표 카드 계산이 다르므로 그 방에서 나간 뒤 새 방을 만들어야 합니다.</p></RuleBlock>
      </>,
    },
  ];
  ...
  return <div className="manual-backdrop" role="dialog" aria-modal="true" aria-label="온라인 게임 규칙 설명">
    <section className="manual-panel updated-manual paged-manual">
      <header><div><p className="eyebrow">ONLINE RULES 4.5 · PAGE {String(page + 1).padStart(2, "0")}/{String(pages.length).padStart(2, "0")}</p><h2>{current.title}</h2><p>{current.subtitle}</p></div><button type="button" onClick={onClose} aria-label="닫기">×</button></header>
