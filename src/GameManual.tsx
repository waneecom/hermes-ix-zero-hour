...
const institutions = [
  [1, "제1 메인 리액터실", "ϟ ϟ"], [2, "양자 연산 코어실", "◉ ⬡"], [3, "중력 제어 장치실", "ϟ ⬡"],
...
export default function GameManual({ onClose }: { onClose: () => void }) {
  ...
  const pages: Array<{ short: string; title: string; subtitle: string; body: ReactNode }> = [
    ...
    },
    {
      ...
      body: <>
        ...
        <RuleBlock title="광물은 5종, 전체 30개입니다"><p>다섯 종류가 각각 정확히 6개씩 있습니다. 기관 카드 13장은 각 2개, 역할 카드 4장은 각 1개를 가집니다. 설명과 실제 게임은 아래의 같은 기호를 사용합니다.</p><div className="manual-minerals">{minerals.map(([icon, name, text]) => <span key={name}><b>{icon}</b><strong>{name} · 전체 6개</strong><small>{text}</small></span>)}</div><p className="rule-note">중앙 목표 카드에는 광물 2개가 있지만 누구도 그 카드를 손에 들지 않습니다. 따라서 네 사람의 카드 4장씩을 합치면 항상 28개가 보입니다. 내 정보의 “◉ 3/6”은 전체 센서 결정 6개 중 내가 3개를 가졌다는 뜻입니다.</p></RuleBlock>
        ...
        <RuleBlock title="바뀌는 것과 바뀌지 않는 것"><p><b>매 게임 무작위:</b> 역할을 받는 사람, 중앙 파괴 목표 1장, 각자 받는 안전 기관 카드 3장, 카드가 화면에 나오는 순서.</p><p><b>항상 고정:</b> 기관별 광물, 역할별 광물, 전체 광물 30개.</p><p className="rule-note"><b>고정 역할 광물:</b> 조종사 ϟ 1개 · 과학자 ✦ 1개 · 보안 책임자 ◆ 1개 · 스파이 ⬡ 1개</p></RuleBlock>
    ...
    },
    {
      ...
      body: <>
        ...
        <RuleBlock title="수석 과학자 — 현장 확인"><p>짝수 라운드에 구역 하나를 고릅니다. 스파이가 그 라운드에 파괴 공격을 했고, 고른 곳이 진짜 목표라면 <b>O</b>입니다. 그 외에는 <b>X</b>입니다.</p><ul><li>O는 고른 구역이 진짜 목표이며 이번 라운드에 공격받았다는 강력한 단서입니다.</li><li>모든 플레이어의 광물 총합은 항상 7개이므로, 의미 없는 “스파이 총합 7개” 공개는 사용하지 않습니다.</li><li>목표 구역이 맞아도 스파이가 조용히 있었다면 X입니다.</li><li>조종사가 공격을 막았더라도 스파이가 목표를 공격했다면 현장 확인은 O가 됩니다.</li></ul></RuleBlock>
    ...
    },
    {
      ...
      body: <>
        ...
        <RuleBlock title="왜 7회가 밸런스가 되나요?"><p>승무원은 매 라운드 세 번 조사할 수 있지만 체포 답 두 개 중 하나만 틀려도 즉시 패배합니다. 스파이는 7회의 성공 공격이 필요하지만, 조사 기록으로 상대 기관 카드 3장을 알아내면 역추적으로 승무원을 제거할 수 있습니다. 조종사는 짝수 라운드에만 방어하고 같은 구역을 연속으로 잠글 수 없어 홀수 라운드에는 반드시 공격 기회가 생깁니다.</p><p className="rule-note"><b>권장 점검:</b> 10경기 후 스파이 승률이 35% 미만이면 7회를 유지한 채 역추적 실패의 정체 공개를 “다음 라운드까지만”으로 완화하고, 65%를 넘으면 역추적 기회를 게임당 1회로 제한하는 편이 다른 규칙을 흔들지 않습니다.</p></RuleBlock>
        <RuleBlock title="이전 규칙 방이라는 안내가 나와요"><p>규칙 4.4 이전에 만든 방입니다. 광물 표시와 파괴 승리 기준이 다르므로 그 방에서 나간 뒤 새 방을 만들어야 합니다.</p></RuleBlock>
      </>,
    },
  ];
  ...
  return <div className="manual-backdrop" role="dialog" aria-modal="true" aria-label="온라인 게임 규칙 설명">
    <section className="manual-panel updated-manual paged-manual">
      <header><div><p className="eyebrow">ONLINE RULES 4.4 · PAGE {String(page + 1).padStart(2, "0")}/{String(pages.length).padStart(2, "0")}</p><h2>{current.title}</h2><p>{current.subtitle}</p></div><button type="button" onClick={onClose} aria-label="닫기">×</button></header>
